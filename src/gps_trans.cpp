...
{
    // if (sensor_msgs::NavSatStatus::STATUS_FIX == msg->status.status)
    // {
        // SIA_DEBUG("*******************************");
        // SIA_DEBUG("Current GPS State:%d", msg->status.status);
...
void odomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
    ...
    tmpMsg = *msg;
    gps_transform.addLidarOdom(tmpMsg);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber lidar_odom_sub = nh.subscribe<nav_msgs::Odometry>("/lio_sam/mapping/odometry", 1, odomCallback);
